...
@section('header_styles')
    <style>
        .blog-hero {
            ...
            padding: 28px 32px;
            border-radius: 24px;
            background: linear-gradient(150deg, rgba(37, 99, 235, 0.18) 0%, rgba(56, 189, 248, 0.12) 55%, rgba(99, 102, 241, 0.2) 100%);
        }
        ...
        html[data-theme='dark'] .blog-hero {
            background: linear-gradient(150deg, rgba(59, 130, 246, 0.2), rgba(37, 99, 235, 0.12));
        }
        ...
        .blog-sidebar {
            ...
            box-shadow: 0 24px 40px -28px rgba(15, 23, 42, 0.12);
        }
        ...
        html[data-theme='dark'] .blog-sidebar {
            ...
            box-shadow: 0 28px 50px -30px rgba(15, 23, 42, 0.65);
        }
        ...
        .composer-card:hover {
            ...
            box-shadow: 0 26px 48px -32px rgba(56, 189, 248, 0.35);
        }
        ...
        html[data-theme='dark'] .composer-heading strong,
        html[data-theme='dark'] .composer-heading p {
...
@section('content')
    @php
        ...
        $defaultComposerTags = ['#coaching', '#training', '#team'];
    @endphp

    <div class="blog-hero">
        <div>
            <h1>Coach Blog — Posts</h1>
            <p>Share coaching insights, training tips, and team updates with your players and the community.</p>
        </div>
    ...
    <div class="blog-layout">
        <div>
            <div class="composer-card" data-coach-name="{{ $coachName }}">
                <div class="composer-heading">
                    ...
                    <div class="w-100">
                        ...
                        <p>Share coaching insights, training tips, or team updates with your players.</p>
                    </div>
                </div>
                <form id="inlineBlogForm" action="{{ route('coach.blog.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    ...
                    <input type="hidden" name="is_public" id="composerIsPublic" value="1">
                    <div class="mb-3">
                    ...
                    <input type="file" class="d-none" id="composerAttachment" name="image" accept="image/*,video/*">
                ...
                <div class="composer-controls">
                    <div class="composer-visibility" role="group" aria-label="Visibility toggle">
                        ...
                        <button type="button" data-visibility="0">Private</button>
                    </div>
            ...
            <div class="feed-stream" data-feed-stream>
                <div class="feed-grid" id="coachFeedGrid">
                    @forelse ($blogs as $blog)
                        ...
                        @endphp
                        <article class="post-card" data-coach="coach">
                            <div class="post-header">
                                <div class="post-author">
                                    <div class="post-avatar">
                                        @if (!empty($user?->photo))
                                            <img src="{{ asset('storage/coaches/' . $user->photo) }}" alt="{{ $user->name }}" style="width: 100%; height: 100%; object-fit: cover; border-radius: 999px;">
                                        @else
                                            {{ Str::of($user->name ?? 'C')->substr(0, 2)->upper() }}
                                        @endif
                                    </div>
                                    <div class="post-meta">
                                        <strong>{{ $user->name ?? 'Coach' }}</strong>
                                        <small @if ($createdAtExact) title="{{ $createdAtExact }}" @endif>{{ $createdAtHuman }}</small>
                                    </div>
                                </div>
                            ...
                            <div class="post-actions">
                                <button
                                    ...
                                    data-like-url="{{ route('coach.blog.like', $blog->id) }}"
                                ...
                                <button
                                    ...
                                    class="post-action"
                                    data-share-button
                                    data-share-url="{{ route('coach.blog.show', $blog->id) }}"
                                    data-share-title="{{ $blog->title ?: 'Coach Update' }}">
                                    <span>Share</span>
                                </button>
                                <a href="{{ route('coach.blog.edit', $blog) }}" class="post-action">
                                    <span>Edit</span>
                                </a>
                            </div>
                            <div
                                ...
                                data-comments-panel
                                data-blog-id="{{ $blog->id }}"
                                data-comments-url="{{ route('coach.blog.comments.index', $blog->id) }}"
                                data-comment-submit-url="{{ route('coach.blog.comments.store', $blog->id) }}"
                                hidden>
                    ...
                    @empty
                        <div class="empty-feed">
                            <h3>No posts yet</h3>
                            <p>You haven't shared anything yet. Start by creating your first coaching post!</p>
        ...
        <aside class="blog-sidebar">
            <div class="sidebar-widget">
                ...
                @foreach($blogs->take(5) as $recent)
                    <a class="sidebar-item" href="{{ route('coach.blog.show', $recent->id) }}">
                        {{ Str::limit($recent->title, 32) }}
                    </a>
                @endforeach
            </div>
            <div class="sidebar-widget">
                <h3>Ideas</h3>
                <a class="sidebar-item" href="#">Training session plan</a>
                <a class="sidebar-item" href="#">Team motivation tips</a>
                <a class="sidebar-item" href="#">Game strategy insights</a>
                <a class="sidebar-item" href="#">Player development</a>
            </div>
        </aside>
    </div>
@endsection

@include('coach.partials.blog-create-modal', ['defaultTags' => $defaultComposerTags])
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const modalEl = document.getElementById('coachBlogModal');
            ...
            const setVisibility = (value) => {
                ...
                const isPublicField = document.getElementById('composerIsPublic');
                if (isPublicField) {
                    isPublicField.value = value;
                }
            ...
            function buildCommentElement(comment) {
                ...
                author.textContent = comment?.name || 'Coach';
            ...
            async function sharePost(button) {
                ...
                try {
                    ...
                    if (fallback !== null) {
                        alert('Link ready! Share it with your team.');
                    }
                } catch (error) {
            ...
            publishBtn?.addEventListener('click', async () => {
                ...
                try {
                    const response = await fetch(@json(route('coach.blog.store')), {
                        method: 'POST',
                        headers: {
