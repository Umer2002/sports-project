...
<div class="row clearfix">
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
        <div class="card">
            ...
            <div class="body">
                <form method="POST" action="{{ isset($club) ? route('admin.clubs.update', $club) : route('admin.clubs.store') }}" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="form-group">
                        <div class="form-line">
                            <input type="text" name="name" class="form-control" value="{{ old('name', $club->name ?? '') }}" required placeholder="Enter club name">
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        <div class="form-line">
                            <input type="email" name="email" class="form-control" value="{{ old('email', $club->email ?? '') }}" required placeholder="Email address">
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        <div class="form-line">
                            <input type="text" name="phone" class="form-control" value="{{ old('phone', $club->phone ?? '') }}" placeholder="Phone number">
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        <div class="form-line">
                            <input type="url" name="paypal_link" class="form-control" value="{{ old('paypal_link', $club->paypal_link ?? '') }}" placeholder="https://paypal.me/your-link">
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        <div class="form-line">
                            <input type="url" name="joining_url" class="form-control" value="{{ old('joining_url', $club->joining_url ?? '') }}" placeholder="https://yourclub.com/join">
                        </div>
                    </div>
                    ...
                    @php
                        ...
                        $existingLinks = isset($club) ? json_decode($club->social_links, true) : [];
                    @endphp
                    @foreach($socials as $platform)
                        <div class="form-group">
                            <label>{{ ucfirst($platform) }}</label>
                            <div class="form-line">
                                <input type="url" name="social_links[{{ $platform }}]" class="form-control" value="{{ old("social_links.$platform", $existingLinks[$platform] ?? '') }}" placeholder="https://{{ $platform }}.com/yourpage">
                            </div>
                        </div>
                    @endforeach
                    ...
                    <div class="form-check m-l-10 mb-4">
                        <label class="form-check-label">
                            <input type="checkbox" name="is_registered" class="form-check-input" value="1" {{ old('is_registered', $club->is_registered ?? false) ? 'checked' : '' }}>
                            <span class="form-check-sign"><span class="check"></span></span>
                            Is Registered
                        </label>
                    </div>

                    <button type="submit" class="btn btn-success waves-effect">{{ isset($club) ? 'Update' : 'Create' }} Club</button>
                </form>
            </div>
        </div>
    </div>
