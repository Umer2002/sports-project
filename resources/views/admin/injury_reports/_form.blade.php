...
<form action="{{ $action }}" method="POST" enctype="multipart/form-data" id="injuryForm">
    @csrf
    @if($method === 'PUT')
        @method('PUT')
    @endif
    ...
    <div id="injury-wizard">
        ...
        <div class="wizard-step step-1">
            @include('admin.injury_reports.steps.step1', ['report' => $report])
        </div>

        <div class="wizard-step step-2 d-none">
            @include('admin.injury_reports.steps.step2', ['report' => $report])
        </div>

        <div class="wizard-step step-3 d-none">
            @include('admin.injury_reports.steps.step3', ['report' => $report])
        </div>

        <div class="wizard-step step-4 d-none">
            @include('admin.injury_reports.steps.step4', ['report' => $report])
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const stepTabs = document.querySelectorAll('.step-nav .step');
        ...
        function showStep(index) {
            ...
            stepTabs.forEach((tab, i) => tab.classList.toggle('active', i === index));
        ...
        submitBtn?.addEventListener('click', function () {
            ...
            .then(async response => {
                if (response.status === 422) {
                    ...
                    window.location.href = "{{ route('admin.injury_reports.index') }}"; // redirect on success
                } else {
